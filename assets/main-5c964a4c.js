import{a as E,i as y}from"./vendor-f9c1c104.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))t(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&t(r)}).observe(document,{childList:!0,subtree:!0});function a(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function t(i){if(i.ep)return;i.ep=!0;const o=a(i);fetch(i.href,o)}})();function b(e){const s=document.querySelector(".loader-container");e&&s.classList.remove("hidden"),s.classList.add("hidden")}document.addEventListener("DOMContentLoaded",function(){b(!0);const e=window.location.pathname.split("/").pop();document.querySelectorAll(".header-nav-list-item a").forEach(function(a){const t=a.getAttribute("href").split("/").pop();e===t||e===""&&t==="index.html"?(a.classList.add("header-nav-current"),b(!1)):(a.classList.remove("header-nav-current"),b(!1))})});(()=>{const e=document.querySelector(".js-menu-container"),s=document.querySelector(".js-open-menu"),a=document.querySelector(".js-close-menu"),t=()=>{const i=s.getAttribute("aria-expanded")==="true"||!1;s.setAttribute("aria-expanded",!i),e.classList.toggle("is-open")};s.addEventListener("click",t),a.addEventListener("click",t),window.matchMedia("(min-width: 768px)").addEventListener("change",i=>{i.matches&&(e.classList.remove("is-open"),s.setAttribute("aria-expanded",!1))})})();function O(){const e=document.getElementsByClassName("exercises-tiles-category-list"),s=document.getElementById("myModal"),a=document.getElementsByClassName("main-modal__close")[0];let t=localStorage.getItem("savedExercises")?JSON.parse(localStorage.getItem("savedExercises")):[];a&&(a.onclick=function(){s.style.display="none",localStorage.removeItem("currentExercise")}),window.onclick=function(r){r.target==s&&(s.style.display="none",localStorage.removeItem("currentExercise"))};for(let r=0;r<e.length;r+=1)e[r].addEventListener("click",function(p){const f=p.target.closest(".exercises-category-tile-button"),h=p.target.closest(".exercises-category-tile-item");if(f){const c=h.dataset.id;s.style.display="flex";const l=`https://your-energy.b.goit.study/api/exercises/${c}`;E.get(l).then(n=>{if(n)return localStorage.setItem("currentExercise",JSON.stringify(n.data)),n.data}).then(n=>{i(n);const m=document.getElementById("fav-btn");if(m){const L=t.some(_=>_._id===n._id);m.querySelector(".main-modal__heart-icon use").setAttribute("href",L?"./img/icons.svg#icon-trash":"./img/icons.svg#icon-heart"),m.querySelector(".main-modal__btn-text").textContent=L?"Unfavorite":"Add to favorites",m.addEventListener("click",function(){const _=t.some(w=>w._id===n._id);_?t=t.filter(w=>w._id!==n._id):t.push(n),localStorage.setItem("savedExercises",JSON.stringify(t)),m.querySelector(".main-modal__btn-text").textContent=_?"Add to favorites":"Unfavorite",m.querySelector(".main-modal__heart-icon use").setAttribute("href",_?"./img/icons.svg#icon-heart":"./img/icons.svg#icon-trash")})}}).catch(n=>{console.error("There was a problem with the Axios request:",n)})}});function i(r){const u=document.getElementById("exerciseDetails");let p=null;r.gifUrl?p=`<div class="main-modal__gif-wrapper">
        ...
        </div>`;const f=`<p class='main-modal__card-title'>${r.name.charAt(0).toUpperCase()+r.name.slice(1)}</p>`,h=o(Math.round(r.rating*10)/10),c=`<div class='main-modal__rating-container'>
      ...
      <use href="./img/icons.svg#icon-star"></use>
        ...
        </li>`).join("");d.tilesCategoryList.innerHTML=i,M(s,a,e)}function M(e,s,a){console.log("totalPages:",s),d.paginationList.innerHTML="";for(let t=1;t<=s;t++){const i=document.createElement("span");i.classList.add("exercises-pagination-item"),i.textContent=t,t==e&&i.classList.add("exercises-pagination-item-active"),i.addEventListener("click",()=>{g.page=t,S(a,g)}),d.paginationList.appendChild(i)}}function x(e){return e[0].toUpperCase()+e.slice(1)}const D=document.querySelector(".quote-wrapper");async function F(){const e=new Date().toLocaleDateString(),s=N(),a=JSON.parse(s);if(!a||a.date!=e){let{quote:t,author:i}=await H();k(t,i),U(t,i,e)}else k(a.quote,a.author)}function U(e,s,a){const t={quote:e,author:s,date:a};localStorage.setItem("quoteOfDay",JSON.stringify(t))}function N(){return localStorage.getItem("quoteOfDay")}async function H(){try{const{quote:e,author:s}=await C("quote","");return{quote:e,author:s}}catch(e){A(e.message)}}function k(e,s){const a=`<p class="quote quote-text">${e}</p>
    <p class="qoute-author">${s}</p>`;D.insertAdjacentHTML("beforeend",a)}function A(e){y.error({message:`${e}`})}F();const R=document.querySelector(".footer-sub-form"),W="https://your-energy.b.goit.study/api/";R.addEventListener("submit",z);async function z(e){e.preventDefault();const s=e.target.email.value;try{const{data:a}=await E.post(`${W}subscription`,{email:s});J(a.message)}catch(a){A(a.response.data.message)}}function J(e){y.success({message:`${e}`})}function Q(e,s=300){let a;return(...t)=>{clearTimeout(a),a=setTimeout(()=>{e.apply(this,t)},s)}}function I(e){e.classList.remove("visually-hidden")}function $(e){e.classList.add("visually-hidden")}const v={searchClearBtnEl:document.querySelector(".js-search-form--btn__clear"),searchInputEl:document.querySelector(".js-search-form--input"),searchIconEl:document.querySelector(".js-search-form--btn__search"),searchClearEl:document.querySelector(".js-search-form--btn__clear"),searchFormEl:document.querySelector(".js-search-form")},G=Q(e=>K(e),400);v.searchClearBtnEl.addEventListener("click",V);v.searchInputEl.addEventListener("input",G);v.searchFormEl.addEventListener("submit",Z);function K({target:e}){e!=null&&e.value.trim()?(I(v.searchClearEl),$(v.searchIconEl)):($(v.searchClearEl),I(v.searchIconEl))}function V(){v.searchInputEl.value="",$(v.searchClearEl),I(v.searchIconEl)}function Z(e){e.preventDefault()}
//# sourceMappingURL=main-5c964a4c.js.map
